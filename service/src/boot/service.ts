...
export async function bootService(cfg: Config, opts: { installSignalHandlers?: boolean } = {}): Promise<RunningService> {
  ...
  // Settle the database moves a dead process left behind BEFORE placement is
  // read: what the restore below lists as "ours" must already be decided.
  const cells = createCells(cfg);
  const moves = createMoves({ cfg, registry, manager, limiter, sync, txRegistry, relay: cells.relay });
  await moves?.sweep();
